...
public class SQL {
    ...
    public void addVoc(String ger, String eng, String eng2) {
        try (Statement statement = connection.createStatement()){
            statement.executeUpdate("INSERT INTO vocabulary (ID, word, translation, translation2, field) VALUES (0, '" + ger + "', '" + eng + "', '" + eng2 + "', 0)");
        } catch (Exception e) {
    ...
    public boolean checkAnswer(int vocID, String answer) {
        if (answer != null && !answer.equals("") && !answer.equals("null")) {
            try (Statement statement = connection.createStatement()) {
                ResultSet resultSet = statement.executeQuery("SELECT * FROM dictionary WHERE ID = " + vocID);
                resultSet.next();

                if (answer.equalsIgnoreCase(resultSet.getString(3))|| answer.equalsIgnoreCase(resultSet.getString(4))) {
                    if (resultSet.getInt(5) < 4){
                        statement.executeUpdate("UPDATE dictionary SET field = " + (resultSet.getInt(5) + 1) + " WHERE ID = " + vocID);
                    }
                    return true;
                } else {
                    if (resultSet.getInt(5) != 0){
                        statement.executeUpdate("UPDATE dictionary SET field = 0 WHERE ID = " + vocID);
                    }
                    return false;
                }
        ...
        //Putting out a random vocabulary as Vocabulary Object  //has to be reworked |comment: change the probability add date
    public Vocabulary getRandomVoc() {
        try (Statement statement = connection.createStatement()){
            ResultSet length = statement.executeQuery("SELECT count(*) FROM dictionary");
            length.next();
            ...
            */
            ...
            while (vocID == 0) {
                ...
                if (r > 40) {
                    ResultSet IDfield0 = statement.executeQuery("SELECT ID FROM dictionary WHERE field = 0 ORDER BY RAND()");
                    if (IDfield0.next()) vocID = IDfield0.getInt(1);
                } else if (r > 16) {
                    ResultSet IDfield1 = statement.executeQuery("SELECT ID FROM dictionary WHERE field = 1 ORDER BY RAND()");
                    if(IDfield1.next()) vocID = IDfield1.getInt(1);
                } else if (r > 6) {
                    ResultSet IDfield2 = statement.executeQuery("SELECT ID FROM dictionary WHERE field = 2 ORDER BY RAND()");
                    if (IDfield2.next()) vocID = IDfield2.getInt(1);
                } else if (r > 3) {
                    ResultSet IDfield3 = statement.executeQuery("SELECT ID FROM dictionary WHERE field = 3 ORDER BY RAND()");
                    if (IDfield3.next()) vocID = IDfield3.getInt(1);
                } else if (r > 0) {
                    ResultSet IDfield4 = statement.executeQuery("SELECT ID FROM dictionary WHERE field = 4 ORDER BY RAND()");
                    if(IDfield4.next()) vocID = IDfield4.getInt(1);
                }
            }
            ResultSet result = statement.executeQuery("SELECT * FROM dictionary WHERE ID =" + vocID);
            result.next();
        ...
        //puts out all vocabularies in a Vocabulary ArrayList   //working
    ...
    public void delVoc(int VocID) {
        try (Statement statement = connection.createStatement()){
            statement.executeUpdate("DELETE FROM dictionary WHERE ID = " + VocID);
        } catch (Exception e) {
